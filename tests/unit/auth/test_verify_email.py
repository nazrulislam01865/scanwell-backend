...
@pytest.mark.asyncio
async def test_verify_email_rejects_wrong_code_without_creating_session() -> None:
    users = InMemoryUserRepository()
    codes = InMemoryVerificationCodeRepository()
    email = FakeEmailService()
    tx = FakeTransaction()
    ...
    with pytest.raises(InvalidVerificationCodeError):
        await verify.execute(VerifyEmailCommand(email="nazrul@example.com", code="000000"))
